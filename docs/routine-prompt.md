...
- **2026-09-15 追記**（ドライランと整合性レビューの指摘への対応。上の 1〜5 の中の文言の明確化）: 前回モードは表の読める最新のレポートから読む / 「注意」行はモードが配信死亡モードでないときだけ / 手順 1 a) c) の「直近 14 日」は今日より前の 14 日 / 独自の「危機モード」や IG・YT を混ぜた平均を使わない / 構造実験の合格ラインの既定 = 続行域 / 手順 7 の hints を IG・YT 別に書く
...
- 同節の集計コマンドを、S と F を台帳の値にして実行する (TODAY は自動で入る。日付を手で書かない。IG と YT で S / F が違うときは行ごとに実行する)。判定窓・経過日・次の判定日と、**IG views 中央値・IG 保存合計**・**YT views 中央値** が出る。n は IG / YT 別。**IG と YT を足したり平均したりしない**
- IG のモードは常に `対象外`。YT は、前回モード (`docs/pdca/` の `YYYY-MM-DD.md` のうち、今日より前の日付で「ジャンル試行の状態」表のモード列が読める最新のもの。1 本も無ければ台帳の「導入時の判定」) から同節の「モードの決め方」で今日のモード (通常 / 切替候補 / 配信死亡モード) を決める。**モードが変わるのは判定日と遅延判定のときだけ** (判定値は毎日出すが、判定日以外は前回モードのまま。今日の判定値が配信死亡なのにモードが `配信死亡モード` でないときだけ「注意」行を書く)
- 過去のレポートや `data/optimization-hints.json` に残っている独自の「危機モード」や、IG と YT を混ぜた平均 (曜日平均など) は判断に使わない。モードはこの 0) で決めたものだけ
...
**a) 過去パフォーマンス**
- `data/performance-history.json` から今日より前の直近 14 日 (今日 − 14 〜 今日 − 1) の `stats.views` (YT) / `instagram.views`・`instagram.saved` (IG。`instagram` か `instagram.views` が null の回は IG 側から除外) / `stats.likes` / `title` / `discovery.method` / `hashtags` / `languages` を抽出 (これはネタ・method 分析用の直近 14 日で、0) の判定窓とは別。method 比較は型の初回投稿日 F 以降の回だけで行う)
...
- a) と同じ直近 14 日の entries を `discovery.method` でグループ化
...
**d) 今日の改善アクションを 3 つまで** (戦略のコンテンツ柱比率、勝ち筋 method 継続 or 新 method 試行を考慮。**YT が配信死亡モードの間、YT については method のアクションを書かず、構造実験 (タイトル個別化 / 型変更 / ジャンル変更。いずれも IG の投稿内容を変えない方法に限る) を「何を変えるか / 何で測るか / 14 日後の合格ライン」で提案する** (合格ラインの既定は `docs/strategy.md` の閾値表の「続行」域 = YT views 中央値 ≥ 20)。構造実験の提案と次の試行候補は**レポートに書くだけ**で、手順 5 の JSON には反映しない。Web 調査に時間を使わず短く書く)
...
`data/performance-history.json` から読み取った傾向を `data/optimization-hints.json` に反映。IG と YT の数字は別々に書き、混ぜた平均を作らない。YT が配信死亡モードの間は、YT views を method の良し悪しの根拠として書かない。
